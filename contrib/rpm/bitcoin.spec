...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 13840
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 33888
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 38888
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 13840
	%{_sbindir}/semanage port -d -p tcp 33888
	%{_sbindir}/semanage port -d -p tcp 38888

Workcoin Core version 0.13.2 is now available from:
  ...
  <https://download.work.provigen.net/workcoin-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a workcoin
...
This allows running workcoind without having to do any manual configuration.
...
`workcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /workcoinrpc>
        ...
        # optional bypass workcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< workcoinrpc:<password>
    </Location>
...
Restarting workcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in workcoin.conf
...
workcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/workcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
- `workcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `workcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of workcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of workcoins being spent is exactly
...
- `workcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `workcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting

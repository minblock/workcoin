Workcoin Core version 0.18.1 is now available from:

  <https://download.work.provigen.net/workcoin-0.18.1/>
...
`/Applications/Workcoin-Qt` (on Mac) or `workcoind`/`workcoin-qt` (on
Linux).
...
Workcoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Workcoin Core on unsupported systems.
...
10.10. Additionally, Workcoin Core does not yet change appearance when
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:13840:13840` (this is an extra `:13840` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@work.provigen.net) the Workcoin Core
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Workcoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Workcoin Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Workcoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Workcoin Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart workcoind with the `-deprecatedrpc=generate`
...
- A new `workcoin-wallet` tool is now distributed alongside Workcoin
...
This section describes planned changes to Workcoin Core that may affect
other Workcoin software and services.

- Since version 0.16.0, Workcoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Workcoin Core 0.20 (expected about
  a year after 0.18), Workcoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Workcoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Workcoin Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Workcoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Workcoin Core
...
- A sub-project of Workcoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Workcoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Workcoin Core's own implementation, although entropy
  gathered by Workcoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Workcoin Core a
...
- On macOS, Workcoin Core now opts out of application CPU throttling
...
- #13840 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
